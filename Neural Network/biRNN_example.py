...
weight = {'out':tf.Variable(tf.random_normal([2 * rnn_hidden,num_class]))}
...
def bi_rnn_model(x, weight, bias):
    ...
    output, _,_ = rnn.static_bidirectional_rnn(lstm_cell_forward, lstm_cell_backward, x, dtype=tf.float32)

    return tf.matmul(output[-1], weight['out'])+bias['out']

logits = bi_rnn_model(input_images, weight,bias)
